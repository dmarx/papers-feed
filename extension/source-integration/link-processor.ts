...
export class LinkProcessor {
   ...
   */
  private injectAnnotationButton(
    link: HTMLAnchorElement,
    sourceId: string,
    paperId: string
  ): void {
    // Create annotator button if not already present
    if (link.nextSibling?.nodeType === Node.ELEMENT_NODE &&
    ...
    annotator.addEventListener('click', (e) => {
      ...
      // Tell background script to show popup
    ...
    logger.debug(`Added annotator button for ${sourceId}:${paperId}`);
  }
