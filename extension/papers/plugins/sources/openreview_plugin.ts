    ...
    try {
      ...
      try {
        ...
        if (forumResponse.ok) {
          ...
          const replies = forumData.notes.filter((n: any) => n.id !== id);
          
          if (replies.length > 0) {
            // Find review notes
            const reviews = replies.filter((n: any) => 
            ...
            const decisions = replies.filter((n: any) => 
            ...
            sourceSpecificMetadata.review_info = {
              ...
              // Extract ratings if available
              ratings: reviews
                .filter((r: any) => r.content.rating || r.content.score || r.content.confidence)
                .map((r: any) => ({
...
pluginRegistry.register(openreviewPlugin);// extension/papers/plugins/sources/openreview_plugin.ts
