...
class BaseSourceIntegration {
    ...
    async createManualPaperEntry(url, document) {
        try {
            logger$3.debug(`Creating manual paper entry for URL: ${url}`);
            ...
            logger$3.debug('Created manual paper entry', metadata);
            return metadata;
        }
        catch (error) {
            logger$3.error('Error creating manual paper entry', error);
            return null;
        }
    }
}
