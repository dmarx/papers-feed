...
to ``smoothly broken" power laws, i.e. a smoothly connected piecewise (approximately) linear function in a log-log plot.  An extensive empirical evaluation demonstrates that BNSL accurately model and extrapolate the scaling behaviors for each task within a large and diverse set of upstream and downstream tasks, in zero-shot, prompted, and fine-tuned settings. This set includes large-scale vision and unsupervised language tasks, diffusion generative modeling of images, arithmetic, and reinforcement learning.
...
\hspace*{-.04cm}\includegraphics[width=0.836\textwidth]{figures/figure_1/figure_1__wide.png}
%\hspace*{-.04cm}\includegraphics[width=0.755\textwidth]{figures/figure_1/figure_1__wide.png}
...
% The power laws commonly used in previous works are a special case of BNSL with $n=0$.
% Just as a power law appears linear on a log-log plot, BNSL is (roughly) piece-wise linear on a log-log plot, see Figure~\ref{fig:figure_1}.

For a mathematical analysis and explanation of why Equation ~\ref{eq:BNSL} is smoothly piece-wise (approximately) linear function on a log-log plot, see Appendix ~\ref{section:bnsl_analysis}. For a mathematical decomposition of Equation ~\ref{eq:BNSL} into the power law segments that it is composed of (e.g. as in Figure \ref{fig:figure_1}), see Appendix ~\ref{section:decomposition_of_BNSL}.
\iffalse
Analysing Equation ~\ref{eq:BNSL} reveals why these functions are smoothly piece-wise (approximately) linear on a log-log plot. % this is the case.
...
By plotting one such term for different values of $c_i, d_i, f_i$, it is easy to confirm that they influence the shape of the curve as described in the previous paragraph.
\fi

Note that, while an intuition for using such smoothly connected approximately piece-wise linear (in log-log plot) function was that, with enough segments, it could fit well any smooth univariate scaling function, it remained unclear whether BNSL would also {\em extrapolate} well; as we demonstrate below,  it does extrapolate quite accurately in our experiments. Additionally, we find that the number of breaks needed to accurately model an entire scaling behavior is often quite small.
...
While this functional form can be summed with an additional constant representing unimprovable performance to obtain a functional form that is mathematically equivalent to our BNSL with a single break, it is important to note that (i) \citet{2021arXiv210201293H} describes this form  only in the specific context, when exploring how  fine-tuning combined with transfer learning scales as a function of the model size - thus, their functional form contains a break only with respect to the number of model parameters but not with respect to the dataset size (which we do explore); (ii) \citet{2021arXiv210201293H} mentioned this equation in passing and as a result did not try to fit or verify this functional form on any data; (iii) they arrived at it simply via combining the scaling law for transfer (that was the focus of their work) with a scaling law for pretraining data; (iv) they did not identify it as a smoothly broken power law, or note any qualitative advantages of this functional form; (v) they did not discuss the family of functional forms with multiple breaks. 
...
\end{align}
However, non-monotonicity of $y$ as an inverse function $y = g^{-1}(x)$ is ruled out, since that would imply two different values of $x=g(y)$ can be obtained for the single value of $y$; however, this is impossible, since $f(y)$ deterministically  maps each $y$  to a single value of $x$. As a result, M4 cannot express non-monotonic functions.
...
It is easy to see that if $y=g^{-1}(x)$ had an inflection point, then $x = g(y)$ would have it as well. This is because an inflection point is defined as a point $x$ where $f(x)$ changes from concave to convex (or vice versa),  which implies that $g(y)$ changes from convex to concave (or vice versa),  since the inverse of a convex function is concave; the root(s) of $g''(y)$ are the point(s) at which this change occurs.
...
We now show the fits and extrapolations of various functional forms. In all plots here and onward and in the appendix, black points are the points used for fitting a functional form, green points are the held-out points used for evaluating extrapolation of a functional form fit to the black points, and a red line is the BNSL that has been fit to the black points. Please refer to Appendix Section \ref{section:BNSL_fit_details} for further experimental details on fitting BNSL. 

\textbf{Except when stated otherwise}, each plot contains a single break of a BNSL fit to all the points that are smaller (along the x-axis) than the green points.
...
\ref{fig:scaling_laws_benchmark_dataset__ImageNet} in Appendix \ref{section:Plots_of_BNSL_Extrapolations}. To view all plots of M1, M2, M3, and M4 on each of these tasks, see Appendix A.4 of \cite{Alabdulmohsi2022revisiting}. To view all plots of M1, M2, M3, M4, and BNSL (overlaid on the same plot) on each of these tasks, see figures
\ref{fig:scaling_laws_benchmark_dataset_all_extrapolations__birds},
\ref{fig:scaling_laws_benchmark_dataset_all_extrapolations__cifar_100},
\ref{fig:scaling_laws_benchmark_dataset_all_extrapolations__caltech},
\ref{fig:scaling_laws_benchmark_dataset_all_extrapolations__ImageNet} in Appendix \ref{section:Plots_of_All_Extrapolations}.
...
In Section \ref{section:extrapolate_oom__compute}, we additionally show that BNSL accurately extrapolates to scales that are order(s) of magnitude larger than the maximum (along the x-axis) of the points used for fitting.
...
To view all plots of M1, M2, M3, M4, and BNSL (overlaid on the same plot) on each of these tasks, see Figures
\ref{fig:scaling_laws_benchmark_dataset_all_extrapolations__big_bench},
\ref{fig:scaling_laws_benchmark_dataset_all_extrapolations__nmt},
\ref{fig:scaling_laws_benchmark_dataset_all_extrapolations__language_modeling} in Appendix \ref{section:Plots_of_All_Extrapolations}.
...
NMT & upstream test cross-entropy & Dec-only, LM & 10 & 1 & 2.52e-1 ± 0 & 1.03e-2 ± 0 & 3.28e-2 ± 0 & 8.43e-3 ± 0 & \bfseries 7.33e-3 ± 0 \\
NMT & upstream test cross-entropy & Transfmr-Enc, LSTM-Dec & 9 & 1 & 1.90e-1 ± 0 & 1.26e-2 ± 0 & 6.32e-2 ± 0 & 1.26e-2 ± 0 & \bfseries 8.30e-3 ± 0 \\
\end{tabular}
    \caption{
...
\includegraphics[width=0.375\textwidth]{figures/double_descent/double_descent__model_size.png}
\includegraphics[width=0.375\textwidth]{figures/double_descent/double_descent__dataset_size_1.png}

    \caption{
    Extrapolation of BNSL on Double Descent. Both plots are of transformers trained to do neural machine translation via minimizing cross-entropy. Experimental data of left figure is obtained from Figure 8 top of \cite{nakkiran2021deep}; ``Model Width" on the x-axis refers to embedding dimension $d_{model}$ of transformer. Experimental data of right figure is obtained from Figure 11b of \cite{nakkiran2021deep}. The left plot contains two breaks of a BNSL fit to the black points. See Section \ref{section:non-monotonic_scaling} for more details.
    }
    \label{fig:double_descent}
\end{figure*}
\FloatBarrier

We show that BNSL accurately models and extrapolates non-monotonic scaling behaviors that are exhibited by Transformers (\cite{vaswani2017attention}) in double descent \citep{nakkiran2021deep} in Figure \ref{fig:double_descent}. Other functional forms are mathematically incapable of expressing non-monotonic behaviors (as shown in Section \ref{section:math_proofs}).
...
\includegraphics[width=.245\textwidth]{figures/rl/connect_four/connect_four__compute.png}
\includegraphics[width=.245\textwidth]{figures/rl/connect_four/connect_four__data.png}
\includegraphics[width=.245\textwidth]{figures/rl/connect_four/connect_four__parameters.png}
\includegraphics[width=.245\textwidth]{figures/rl/procgen/Procgen_Heist.png}
    \caption{
...
We show that BNSL accurately models and extrapolates the scaling behavior of tasks that have an inflection point on a linear-linear plot such as the task of arithmetic (4-digit addition) in Figure \ref{fig:arithmetic} left. Here we model and extrapolate the scaling behavior of a transformer model (\cite{vaswani2017attention}) with respect to the dataset size on the 4-digit addition task. Various other functional forms are mathematically incapable of expressing inflection points on a linear-linear plot (as shown in Section \ref{section:math_proofs}) and as a result, are mathematically incapable of expressing and modeling inflection points (on a linear-linear plot) that are present in the scaling behavior of 4-digit addition. For further details about the hyperparameters please refer to the Appendix Section \ref{section:Inflection_points_experimental}.
...
\includegraphics[width=0.37\textwidth]{figures/arithmetic/4_digit_addition__dataset_size.png}
%\includegraphics[width=0.48\textwidth]{figures/arithmetic/4_digit_addition__dataset_size__very_first_version.png}
\includegraphics[width=0.37\textwidth]{figures/arithmetic/4_digit_addition__dataset_size__very_first_version__simulation_limit.png}

    \caption{
    Extrapolation of BNSL on 4 Digit Addition. These plots are \textbf{linear-linear}. In left plot, each point is gathered from a model trained to do the task of 4 digit addition and is the mean of greater than 1000 seeds at that dataset size. In right plot, each point is gathered from a noiseless simulation of the BNSL of the task of 4 digit addition. See Sections \ref{section:Inflection_Points}, \ref{section:Inflection_points_experimental}, \ref{section:limit_of_agi_superforecasting} for more details.
    }
...
We use BNSL to glean insights about the limit of the predictability of scaling behavior. Recent papers \citep{ganguli2022predictability, wei2022emergent} have advertised many tasks as having ``unpredictable" ``emergent" scaling behavior, the most famous of which is the task of arithmetic. In the previous Section \ref{section:Inflection_Points} and in Figure \ref{fig:arithmetic} left, we successfully predicted (i.e. extrapolated) the scaling behavior of 4-digit addition (arithmetic). However, we are only able to accurately extrapolate the scaling behavior if given some points from training runs with a training dataset size of at least 720, and the break in which the scaling behavior of 4-digit addition transitions from one power law to another steeper power-law happens at around training dataset size of 415. 

Ideally, one would like to be able to extrapolate the entire scaling behavior by fitting only points from before the break. In Figure \ref{fig:arithmetic} right, we use a noiseless simulation of the BNSL of 4-digit addition to show what would happen if one had infinitely many training runs / seeds to average out all the noisy deviation between runs such that one could recover (i.e. learn via a curve-fitting library such as SciPy \citep{virtanen2020scipy}) the learned constant of the BNSL as well as possible. When using this noiseless simulation, we find that we are only able to accurately extrapolate the scaling behavior if given some points from training runs with a training dataset size of at least 415, which is very close to the break. 
...
An ethical concern one might have about our work is that revealing BNSL might differentially \citep{hendrycks2022x} improve AI capabilities progress relative to AI safety/alignment progress. 
A counter-argument is that BNSL will also allow the AI safety/alignment field to extrapolate the scaling behaviors of its methods for aligning AI systems and as a result will also accelerate alignment/safety progress.
...
%\noindent{\bf Acknowledgement.} We are thankful for useful feedback and assistance from Kartik Ahuja, Ibrahim Alabdulmohsin, Ankesh Anand, Jacob Buckman, Guillaume Dumas, Andy Jones, Behnam Neyshabur, Gabriel Prato, Michael Trazzi,  Tony Wu and others.
...
\begin{figure*}[htbp]
    \centering
%\includegraphics[width=0.48\textwidth]{figures/arithmetic/4_digit_addition__dataset_size.png}
\includegraphics[width=0.48\textwidth]{figures/order_of_magnitude/imagenet_5___MiX_L_16.png}
\includegraphics[width=0.48\textwidth]{figures/order_of_magnitude/imagenet_10___MiX_L_16.png}
\includegraphics[width=0.48\textwidth]{figures/order_of_magnitude/imagenet_25___MiX_L_16.png}

    \caption{
    Extrapolation Results of BNSL to Scales that are an Order of Magnitude larger than the maximum (along the x-axis) of the points used for fitting. Experimental data of scaling behavior obtained from Figure 5 of \cite{abnar2021exploring}. The upstream task is supervised pretraining of MLP mixers (MiX) \citep{tolstikhin2021mlp} on subsets (i.e. the x-axis of plot) of JFT-300M \citep{sun2017revisiting}. The Downstream Task is n-shot ImageNet classification (i.e. the y-axis of plot). See Section \ref{section:extrapolate_oom} for more details.
    }
...
\subsection{Extrapolation to Scales that are Order(s) of Magnitude larger than the maximum (along the x-axis) of the points used for fitting}
\label{section:extrapolate_oom__compute}
...
\scriptsize
    \centering
    \begin{tabular}{|c|Hc|c|c|c|c|} 
\hline
Task & Model & M1 $\downarrow$ & M2 $\downarrow$ & M3 $\downarrow$ & M4 $\downarrow$ & BNSL $\downarrow$\\
 \hline
 %IC & Birds 200 & 0 & 0 & 0 & 5.56 & \highlight{94.44}\\ 
 %IC & Caltech101 & 0 & 0 & 5.56 & 5.56 & \highlight{88.89}\\ 
 %IC & CIFAR-100 & 0 & 5.56 & 5.56 & 5.56 & \highlight{83.33}\\
 ...
 %version where Ibrahim had error %Downstream Image Classification & All & 0\% & 1.39\% & 5.56\% & 4.17\% & \bfseries 88.89\%\\
 %ImageNet 10-Shot (extrap 1 OOM) & ViT & 0.2845 +- 0.03709 & 0.1058 +- 0.01572 & 0.0455 +- 0.00604 & 0.0403 +- 0.00673 & \bfseries 9.79e-3 ± 4.70e-3\\
  ImageNet 10-Shot (1 OOM) & ViT & 2.85e-1 ± 3.7e-2 & 1.06e-1 ± 1.6e-2 & 4.55e-2 ± 6.0e-3 & 4.03e-2 ± 6.7e-3 & \bfseries 9.79e-3 ± 4.7e-3\\
 %version where Ibrahim had error %Language & All & 5\% & 10\% & 15\% & 25\% & \bfseries 45\%\\
 %ImageNet Finetune & ViT & 1.14e-2 ± 2.42e-3 & \bfseries 9.37e-3 ± 2.60e-3 \\
   ImageNet 10-Shot (2 OOM) & ViT & 3.39e-1 +- 5.3e-2 & 3.39e-1 +- 5.3e-2 & 3.39e-1 +- 5.3e-2 & 1.52e-1 +- 1.7e-2 & \bfseries 1.86e-2 ± 3.8e-2\\
 \hline
\end{tabular}
    \caption{
    Extrapolation Results (on Downstream Vision Tasks) to scales that are order(s) of magnitude larger than the maximum (along the x-axis) of the points used for fitting. Training runs are scaled using the compute-optimal scaling (i.e. Pareto frontier) with respect to downstream performance. Experimental data obtained from Figure 2 of \cite{DBLP:journals/corr/abs-2106-04560}. The first row contains RMSLE of extrapolating 1 order of magnitude. The second row contains RMSLE of extrapolating 2 orders of magnitude. See Section \ref{section:extrapolate_oom__compute} for more details.
    }
    \label{table:extrapolate_oom__compute}
\end{table}

\begin{figure*}[htbp]
    \centering
%\includegraphics[width=0.48\textwidth]{figures/arithmetic/4_digit_addition__dataset_size.png}
\includegraphics[width=0.48\textwidth]{figures/order_of_magnitude__compute_x-axis/1oom.png}
\includegraphics[width=0.48\textwidth]{figures/order_of_magnitude__compute_x-axis/2oom.png}

    \caption{
    Extrapolation Results (on Downstream Vision Tasks) to Scales that are order(s) of magnitude larger than the maximum (along the x-axis) of the points used for fitting. The left plot contains extrapolation to scales that are 1 Order of Magnitude larger than the maximum (along the x-axis) of the points used for fitting. The right plot contains extrapolation to scales that are 2 orders of magnitude larger than the maximum (along the x-axis) of the points used for fitting; M1 and M2 are not visible in the right plot because they perfectly overlap with M3. Experimental data of scaling behavior obtained from Figure 5 of \cite{abnar2021exploring}. See Section \ref{section:extrapolate_oom__compute} for more details.
    }
    \label{fig:extrapolate_oom__compute}
\end{figure*}

In Figure \ref{fig:extrapolate_oom__compute}, we show that BNSL accurately extrapolates to scales that are order(s) of magnitude larger than the maximum (along the x-axis) of the points used for fitting; compute (FLOPs) used for (pre-)training is on the x-axis and compute is scaled in the manner that is Pareto optimal with respect to the performance evaluation metric (downstream error rate in this case). The experimental scaling data was obtained from Figure 2 of \cite{DBLP:journals/corr/abs-2106-04560}. The upstream task is supervised pretraining of Vision Transformers (ViT) \citep{dosovitskiy2020image} on subsets (i.e. the x-axis of plot) of JFT-300M \citep{sun2017revisiting}. The downstream task is 10-shot ImageNet classification (i.e. the y-axis of plot). M1, M2, M3, and M4 were fit using the fitting code provided by \cite{Alabdulmohsi2022revisiting}. In Table \ref{table:extrapolate_oom__compute} and Figure \ref{fig:extrapolate_oom__compute}, we compare extrapolation of BNSL to the extrapolation of M1, M2, M3, and M4; we find that BNSL yields extrapolations of scaling behavior that are more accurate on these tasks than all of these functional forms.
