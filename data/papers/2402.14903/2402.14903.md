...
<span class="image">Illustrating the dependence of frontier model arithmetic performance on tokenization. We show how using commas can enforce right-to-left (R2L) tokenization for the same addition problem. R2L tokenization leads to improved model performance on both GPT-3.5 and GPT-4 (March 2023 models), which we show is due to tokenization alignment between addends and answer through various controls and error analyses.</span> 
...
<span class="image">Comparison of how `p50k_base`, the tokenizer for GPT-3, and `cl100k_base`, the tokenizer for GPT-3.5 and GPT-4, segments 4 digit strings into tokens. `cl100k_base` standardized number tokenization to chunks of 3 digits, left-to-right, resulting in all N-digit numbers being segmented the same way.</span> 
...
<span class="image">Effect of R2L vs L2R tokenization with increasing shots.</span> 
...
<span class="image">8-shot accuracy when using different delimiters for R2L tokenization. Dotted lines show results from Figure  for comparison. Overall, we see choice of delimiter matters less than direction of tokenization.</span> 
...
<span class="image">8-shot accuracy for various “thinking token” controls. Dotted lines show results from Figure  for comparison. We also experimented with other delimiters for L2R tokenization (all those from Figure ), but found similarly poor results. Overall, “thinking tokens” do not recover the performance boost from using comma-enforced R2L tokenization.</span> 
...
<span class="image">When the answer is the same length in digits as an addend (length match), both tokenization schemes perform similarly (left). When the answer is a different length in digits than either addend (length mismatch), L2R tokenization destroys model performance, dropping to 8.25% (right).</span> 

<span class="image">Accuracy as a function of number of carries. For L2R tokenization, we exclude problems where the answer length does not match at least one addend, as the model misses most of those (92%) as shown in Section . If number of carries (a human notion of difficulty) was correlated to model performance, we would expect a negative slope. The lack of any trend suggests model performance is largely independent of number of carries.</span> 
...
<span class="image">Location of errors</span>

<span class="image">Magnitude of errors</span>
...
<span class="image">Few-shot accuracy when models receive a problem with one tokenization direction, then repeat and answer it in another.</span> 
...
<span class="image">Few-shot accuracy when models receive a problem in one tokenization format, and answer it in another. The distinction between this and Figure  is that models *do not* repeat the problem in this case. We note that when giving a model a problem in R2L tokenization and prompting it to answer in L2R tokenization, the model actually gets *worse* with more shots, since for fewer shots, the model ends up ignoring the few-shot prompt and answers in its preferred R2L tokenization. Specifically, adherence to the prompted formatting for R2L\(\rightarrow\)L2R increases from just 13.3% with 1 shot to 98.9% with 8 shots.</span> 

<span class="image">8-shot performance of various OpenAI models on the same addition problems as Figure . The newer version sof GPT-3.5 appears to perform equally poorly. For GPT-4, we see a large tokenization dependent effect in the March model, which becomes weaker (but still present) in the June model. The GPT-4 turbo model shows a slight regression in overall performance with the tokenization-dependent effect becoming stronger again.</span> 
...
<span class="image">8-shot performance of various OpenAI models on answer length controlled problems (see Section ), separated by whether the answer length is the same as one of the addends. We see the effect from Section  reproduces strongly in the newer version of GPT-3.5. The effect is still present in GPT-4,\[13\] but not as strongly. Interestingly, the effect is stronger in the latest GPT-4 Turbo model as compared to GPT-4.</span> 
...
<span class="image">The equivalent of Figure  but for the Claude tokenizer. All 3-digit number strings, colored red when the string does not have a corresponding single token dedicated to it. The lack of systematicity suggests that Claude tokenizes numbers using pure BPE. Note also, however, that token coverage is generally higher than in Figure , likely in part because the Claude tokenizer has a larger vocabulary size (65k tokens) than OpenAI’s `p50k_base` (50k tokens).</span>

<span class="image">The equivalent of Figure  but for the Claude tokenizer. Note that this distribution looks more like `p50k_base` than `cl100k_base` in Figure . This, along with Figure  above shows that Claude’s tokenizer exhibits a lack of systemacity when tokenizing numbers, suggesting the use of pure BPE number tokens, rather than something bespoke (as other current models use; see Table ).</span>
...
<span class="image">Comparison of R2L and L2R tokenization strategies for different numbers of shots and using two different system prompts.</span>

# Frequency effects

Given the findings of prior work on numerical reasoning demonstrating frequency effects , we also investigated whether or not our observed error patterns could be explained by term frequency. While we do not have access to the pre-training data of GPT-3.5 and GPT-4 models, we use the tokenizer merge ranks\[16\] as a signal of term frequency. We analyze the expanded set of problems used for the error analysis in Section . Our results are summarized below:
...
<span class="image">Distribution of relative rank of substituted incorrect token 2 in model response when using L2R tokenization in the length mismatch condition.</span>
...
&\equiv H_{\text{lower}},\end{aligned}\] where \(p_i\) denotes the probability of the \(i\)-th most likely token. We use the natural logarithm for entropy, so all entropies are in nats (not bits).
...
13. We were unable to run experiments on `gpt-4-0314` for this analysis as the model has been removed from the API, despite a stated deprecation date of June 13, 2024

14. We also considered frequency effects by utilizing BPE merge ranks (given that we do not have access to the pre-training data of GPT-3.5 and GPT-4 models) as an approximate for frequency. We didn’t find a strong effect, expanded results are provided in Appendix .

15. 3-digit number tokens also reduce inference-time compute when models use numbers in their output, which could be an important consideration when serving models at scale.

16. Recall that tokens are created roughly in order of decreasing frequency in the corpus used to train the tokenizer.

17. A completely random guess over 10 tokens would correspond to an entropy of about 2.3, which is in line with the lower bound observed (of about 2.06) and the finding of a slight mangitude bias (which would decrease the entropy from 2.3).

18. A completely random guess over 2 tokens would correspond to an entropy of about 0.69, which is in line with the lower bound observed (of about 0.45).
