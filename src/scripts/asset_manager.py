...
def main():
    ...
    fire.Fire({
        'ensure': PaperAssetManager().ensure_all_assets,
        'retry-markdown': lambda: PaperAssetManager().markdown.retry_failed_conversions(force=True),
        'status': lambda: PaperAssetManager().get_incomplete_assets()
    })
